...
namespace Game.Fight.AI.Actions
{
    public sealed class SummonAIAction : SpellAIActionBase
    {
        ...
        public override bool CanExecute(AIContext context)
        {
            ...
            {
                Logger.Warn("[SummonAIAction] base.CanExecute fallo para fighter=" + (context?.Fighter?.Id ?? 0) + " spellId=" + Decision?.SpellId);
                return false;
            }
            ...
            {
                Logger.Warn("[SummonAIAction] Spell " + Decision?.SpellId + " no tiene efecto de invocacion. Efectos: " + (spell?.Effects == null ? "null" : string.Join(",", spell.Effects.Select(e => (int)e.TypeEnum))));
                return false;
            }
            ...
            {
                Logger.Warn("[SummonAIAction] Limite de invocaciones: actual=" + currentInvocations + " maximo=" + maxInvocations + " fighter=" + (context?.Fighter?.Id ?? 0));
                return false;
            }
